        ...
        PseudoInfiniteViewport.Pannable
{
     ...
     * empty.
     */
    public GridPanel(DisplayMap map, ResourceBundle res, World<?> world)
    {
        displayMap = map;
        resources = res;
        this.world = world;
    ...
    public void paintComponent(Graphics g)
    {
        ...
        //drawWatermark(g2);
        drawAlternatingSquares(g2);//draw the red and green boxes
        drawGridlines(g2);
        ArrayList<Location> validMoveLocs = world.getHighlightLocations();
        drawMoveLocations(g2,validMoveLocs);//highlight the valid move locations
        drawOccupants(g2);//now draw occupants over that
        drawCurrentLocation(g2);
    }
    /**
     * Modified from JChess Link: <a>https://github.com/Ceasar/JChess</a>
     * @param g2
     */
    private void drawAlternatingSquares(Graphics2D g2){
    	//this is terrible and would be terrible broken if used with anything other than a 8X8 bounded grid, yet there is no garuntee here
    	Insets insets = getInsets();
    	...
    	for(int row=0; row < 8; row++){//for each row
    		...
    		for(int col = 0;col<8;col+=2){//for each box in the row
    ...
    /*More modified code*/
    private void drawMoveLocations(Graphics2D g2, ArrayList<Location> locs){
    	for(int i=0; i<locs.size(); i++){
    		Location curLoc = locs.get(i);
    		
    		Point p = pointForLocation(curLoc);
    		Graphics2D g3 = (Graphics2D) g2.create();
    		if(world.isOpponentPieceSelected()){
    			g3.setColor(Color.PINK);
    		}else{
    			g3.setColor(Color.BLUE);
    		}
    		g3.fillRect(p.x - cellSize/2 -2, p.y - cellSize / 2 -2, cellSize + 2, cellSize + 2);
    		Graphics2D g4 = (Graphics2D) g2.create();
    		g4.setColor(Color.YELLOW);
    		g4.drawRect(p.x - cellSize / 2 - 2, p.y - cellSize / 2 - 2,
